...
| [Transformer Training: Teacher Forcing & the Decoder Causal Mask](https://mtgca.github.io/interactive-widgets/transformer_training_interactive.html) | AI / Deep Learning | [@mtgca](https://github.com/mtgca) | ⚠️ AI-only | Follow one translation pair through encoder–decoder training: the target sentence shifted into decoder input and loss target, padding masks for variable-length batches, teacher forcing, and a causal-mask explorer showing how one 8-token sequence becomes 8 parallel training examples under a single cross-entropy loss. |
