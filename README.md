...
| [Why Not Just Flatten the Sequence?](https://mtgca.github.io/interactive-widgets/naive_dense_sequence_cost_interactive.html) | AI / Deep Learning | ✅ Verified | Flatten a sequence into one vector and feed it through a Dense layer — watch the weight matrix explode as N²·d². Motivates why naive fully-connected sequence mixing doesn't scale. |
...
| [Trainable Positional Embeddings: Same Table, Different Index](https://mtgca.github.io/interactive-widgets/positional_embeddings_interactive.html) | AI / Deep Learning | ✅ Verified | Positional embeddings are just embeddings indexed by position, not token: look up both tables, add rows, watch predictions shift by position. |
| [Attention Q, K, V: The Library Analogy](https://mtgca.github.io/interactive-widgets/attention_qkv_library_interactive.html) | AI / Deep Learning | ⚠️ AI-only | Pick a query and watch a library answer it: Keys (spine labels) are matched, matches become attention weights that add up to 100%, and the result blends the Values proportionally — a pure analogy, no equations. |
| [A Self-Attention Head, Step by Step](https://mtgca.github.io/interactive-widgets/self_attention_head_interactive.html) | AI / Deep Learning | ⚠️ AI-only | Follow one attention head on a real computed example: project x into Q, K, V, take scaled dot-products, softmax into attention weights (asymmetric, non-constant diagonal), and blend the Values — then scale up to multi-head attention with dₖ = d/h. |
