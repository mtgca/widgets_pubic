...
| [Trainable Positional Embeddings: Same Table, Different Index](https://mtgca.github.io/interactive-widgets/positional_embeddings_interactive.html) | AI / Deep Learning | ⚠️ AI-only | Positional embeddings are just embeddings indexed by position, not token: look up both tables, add rows, watch predictions shift by position. |
| [Attention Q, K, V: The Library Analogy](https://mtgca.github.io/interactive-widgets/attention_qkv_library_interactive.html) | AI / Deep Learning | ⚠️ AI-only | Pick a query and watch a library answer it: Keys (spine labels) are matched, softmax turns scores into weights, and the result blends the Values proportionally — attention as weighted retrieval, before any math. |
